		...
		methods: {
			async cancelBooking(item){
				...
				if(cancelBooingStatus.code){
					console.log(cancelBooingStatus);
					
					// 传递初始化数据
					sendBookingData.id = item.id;
					sendBookingData.phone= this.userInfo.username;
					
					uni.$emit("handleBookingEvent",sendBookingData,);
